...
poll_input :: proc() {
	...
	if currentScreen == .Game || currentScreen == .Draw { 	//TODO: this is ugly
		...
		for numkey, i in ([]rl.KeyboardKey {
				.ONE,
				.TWO,
				.THREE,
				.FOUR,
...
handle_paused :: proc() -> bool {
	...
	if paused {
		...
		draw_settings({(msg_start.x + msg_end.x) / 2, msg_end.y}, .TOP)
	}
	return paused
}
...
draw_radar :: proc() {
	...
	for m in state.meteors {
		...
		if ms.x >= 0 && ms.x <= screen_end.x && ms.y >= 0 && ms.y <= screen_end.y {
			continue
		}
		...
		draw_dot(
			screen_center + dir * min(tx, ty),
			1.0,
			.PRIMARY,
			2.0 - dist_squared({tx, ty}, ms) * 0.00001,
		)
	}
}
...
draw_title_screen :: proc(delta: f32) {
	...
	draw_text("jcomcl and BurningCactus", {tx_rect.x / 2 - padv.x, draw_start.y}, 17, .CENTER)
