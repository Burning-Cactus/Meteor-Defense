...
draw_vfx :: proc(v: ^Vfx, state: ^GameState) {
	...
	case .BANG:
		...
		draw_circle(v.pos, radius, v.col, scale_hint)
	case .SPARK:
		trail_time: f32 = 0.1
...
spawn_bang :: proc(state: ^GameState, pos: Vec2, radius: f32 = 60.0, lifetime: f32 = 0.5, col: ThemeColor = .YELLOW) {
...
spawn_spark :: proc(state: ^GameState, pos: Vec2, velocity: Vec2, lifetime: f32 = 0.6, col: ThemeColor = .YELLOW) {
	spawn_vfx(state, pos, Vfx{
		type     = .SPARK,
		col      = col,
		draw     = draw_vfx,
...
spawn_sparks :: proc(state: ^GameState, pos: Vec2, count: int = 8, speed: f32 = 200.0, lifetime: f32 = 0.6, col: ThemeColor = .YELLOW) {
	for _ in 0..<count {
		angle := rand.float32() * math.TAU
		spd   := speed * (0.4 + rand.float32() * 0.8)
		vel   := Vec2{math.cos(angle), math.sin(angle)} * spd
		lt    := lifetime * (0.6 + rand.float32() * 0.8)
		spawn_spark(state, pos, vel, lt, col)
	}
}
